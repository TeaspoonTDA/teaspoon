...
class PathOptimizer:
    ...
    def __init__(self, loss_terms, forbidden_regions=None, a=10.0):
        ...
        assert set(loss_terms.keys()).issubset(
            valid_terms), f"Invalid loss terms. Valid terms are: {valid_terms}."
        assert all(sign in [-1, 1] for sign in loss_terms.values()
                   ), "Signs must only contain +1 or -1."
    ...
    def compute_loss(self, pts, params):
        ...
        if 'maxPers' in self.loss_terms:
            ...
            loss += self.loss_terms['maxPers'] * \
                (maxPers1 / (maxPers1.detach() + 1e-8))

        if 'entropy' in self.loss_terms:
            ent1 = PersistentEntropy(
                (diag1[:, 1] - diag1[:, 0]).to(torch.float64), normalize=True)
            loss += self.loss_terms['entropy'] * \
                (ent1 / (ent1.detach() + 1e-8))

        if 'perstot' in self.loss_terms:
            perstot1 = torch.sum((diag1[:, 1] - diag1[:, 0]).to(torch.float64))
            loss += self.loss_terms['perstot'] * \
                (perstot1 / (perstot1.detach() + 1e-8))

        if self.forbidden_regions:
            reg = 0.0
            for region in self.forbidden_regions:
                reg += torch.exp(torch.tensor(self.a,
                                 dtype=torch.float64) * region(params, diag1))
            loss += reg
...
if __name__ == "__main__":
    ...
    # Time settings
    t_main = torch.linspace(0, 10, 1000, device=device,
                            dtype=torch.float64)   # Main simulation
    ...
    forbidden_regions = [
        ...
        # Example constraint: unit disk of radius 5 centered at (190, 27)
        lambda params, pd1: -
        ((1/400)*(params[0]-190)**2+(1/25)*(params[1]-10)**2 - 1.0),
    ]
    p_opt = PathOptimizer({
    ...
    for epoch in range(5):
        ...
        y, pd1, loss, grads = p_opt.optimize(
            lorenz, x0, t_main, optimizer, scheduler)
        ...
        print(
            f"Rho: {lorenz.params[0].item():.8} -- Sigma: {lorenz.params[1].item():.8}")
