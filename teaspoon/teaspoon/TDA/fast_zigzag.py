...
def get_greedy_perm(X, n_perm):
    ...
    dpoint2all = lambda i: dpoint2pointcloud(X, i)
    ds = dpoint2all(0)
    dperm2all = [ds]
...
def clean_data(classes, lines_i, lines_d):
    ...
    for i in classes.keys():
        ...
        for cl in range(len(lines_i)):
            ...
            if cl == 0:
                m = np.logical_and(classes[i]['x'] <= lines_i[cl], classes[i]['x'] >= 0, classes[i]['y'] >= lines_i[cl])
                mask = mask + m
            else:
                m = np.logical_and(classes[i]['x'] <= lines_i[cl], classes[i]['x'] >= lines_i[cl-1], classes[i]['y'] >= lines_i[cl])
                mask = mask + m
...
def plot_data(classes, plotH2=False):
    ...
    for class_label, coordinates in classes.items():
        if plotH2:
            plt.scatter(coordinates['x'], coordinates['y'], label=f'H{int(class_label)}', marker=markers[int(class_label)])
...
def generate_input_file(point_clouds, filename='output', radius=10, n_perm=25, plotting=False):
    ...
    for num in range(len(point_clouds)):
        ...
        if num == 0:
            ...
            elements = get_filtration(np.concatenate((point_clouds[num], point_clouds[num+1])), radius=radius)
            elements_array = np.array([np.array(item) for item in elements], dtype=object)

            mask = np.array([np.any(item >= n_points[0]) for item in elements_array])
            ...
            mask = np.array([np.any(item >= sum(n_points[:num+1])) for item in elements_array])
            filtered_elements = elements_array[mask]
            filtered_elements = filtered_elements.tolist()
