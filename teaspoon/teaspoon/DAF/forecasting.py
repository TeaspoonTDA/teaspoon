def random_feature_map_model(u_obs, Dr, w=0.005, b=4.0, beta=4e-5, seed=None):
    ...
    W_LR = u_obs[:,
                 1:]@phi_mat.T@np.linalg.inv((phi_mat@phi_mat.T+beta*np.eye(Dr)))
...
def G_rfm(xn, W_LR, mu, auto_diff=False):
    ...
    if auto_diff:
        ...
        try:
            ...
            raise ImportError(
                "TensorFlow is required for auto_diff functionality.")
...
def get_forecast(Xp, W, mu, forecast_len, G=G_rfm, auto_diff=False):
    ...
    if not auto_diff:
        ...
        forecast = tf.reshape(tf.convert_to_tensor(
            Xp[:, -p:], dtype=tf.float64), (D, p))
...
def forecast_time(X_model_a, X_truth, dt=1.0, lambda_max=1.0, threshold=0.05):
    ...
    for i in range(1, X_model_a.shape[1]):
        error = np.divide(np.linalg.norm(
            X_model_a[:, 0:i]-X_truth[:, 0:i], axis=1)**2, np.linalg.norm(X_truth[:, 0:i], axis=1)**2)
...
if __name__ == "__main__":
    ...
    t, ts = lorenz(L=500, fs=50, SampleSize=6001, parameters=[
                   28, 10.0, 8.0/3.0], InitialConditions=ICs)
    ts = np.array(ts)

    # Add noise to signals
    noise = np.random.normal(scale=0.01, size=np.shape(
        ts[:, 0:train_len+forecast_len]))
    u_obs = ts[:, 0:train_len+forecast_len] + noise

    # Train model
    W_LR, W_in, b_in = random_feature_map_model(
        u_obs[:, 0:train_len], Dr, seed=r_seed)

    # Generate forecast
    forecast_len = 500
    X_model = get_forecast(u_obs[:, train_len].reshape(-1, 1),
                           W_LR, mu=(W_in, b_in), forecast_len=forecast_len)
