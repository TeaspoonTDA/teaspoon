...
def TADA(u_obs, window_size, model_parameters, n_epochs=1, train_len=4000, opt_params=[1e-5, 0.99], window_number=1, j2_sample_size=50):
    ...
    if len(model_parameters) != 5:
        raise ValueError(
            "model_parameters must contain four items. W_A, W_LR, mu, G, p")
    ...
    X_model = get_forecast(u_obs[:, train_len-p+1:train_len+1], W_A,
                           mu, forecast_len=end-train_len, G=G, auto_diff=True)[:, start:end]
    ...
    for epoch in range(n_epochs):
        ...
        with tf.GradientTape() as tape:
            ...
            forecast = get_forecast(
                start_pt, W, mu, forecast_len=window_number, G=G, auto_diff=True).T

            # Get persistence diagrams from current window forecast
            dgm = rips_layer.call(
                tf.cast(forecast[-current_window_size:, :], dtype=np.float32))
            ...
            distance0 = wasserstein_distance(
                dgm0, target_pd0, order=1., internal_p=2., enable_autodiff=True, keep_essential_parts=False)
            distance1 = wasserstein_distance(
                dgm1, target_pd1, order=1., internal_p=2., enable_autodiff=True, keep_essential_parts=False)

            # Compute error Wasserstein distances
            reg_dgm = rips_layer.call(tf.cast(
                forecast[-current_window_size:, :], dtype=np.float32)-tf.constant(X_meas.T, dtype=np.float32))
            ...
            reg_distance0 = wasserstein_distance(
                reg_dgm0, empty_dgm, order=1., internal_p=2., enable_autodiff=True, keep_essential_parts=False)
            reg_distance1 = wasserstein_distance(
                reg_dgm1, empty_dgm, order=1., internal_p=2., enable_autodiff=True, keep_essential_parts=False)
            ...
            random_indices = tf.random.shuffle(
                tf.range(tf.shape(u_obs[:, :train_len])[0]))[:j2_sample_size]
            training_samples = u_obs[:, :train_len][:, random_indices]
            train_loss = G(training_samples, W, mu,
                           auto_diff=True) - training_samples
            train_dgm = rips_layer.call(
                tf.cast(tf.squeeze(tf.gather(train_loss, random_indices)), np.float32))
            train_dgm0 = tf.cast(train_dgm[0][0], dtype=np.float64)
            train_dgm1 = tf.cast(train_dgm[1][0], dtype=np.float64)
            empty_dgm = tf.constant(
                np.array([]), shape=(0, 2), dtype=np.float64)
            J_2_loss = wasserstein_distance(train_dgm0, empty_dgm, order=1., internal_p=2., enable_autodiff=True, keep_essential_parts=False) + \
                wasserstein_distance(train_dgm1, empty_dgm, order=1., internal_p=2.,
...
def get_initial_pds(X):
    ...
    st = gd.RipsComplex(points=np.transpose(
        X)).create_simplex_tree(max_dimension=2)
    dgm = st.persistence()
...
def rafda(u_obs, Dr, Gamma, M=1000, g=1000, w=0.005, b=4.0, beta=4e-5, seed=None):
    ...
    D = np.shape(u_obs)[0]
    N = np.shape(u_obs)[1]
    W_LR, W_in, b_in = random_feature_map_model(
        u_obs, Dr, w=w, b=b, beta=beta, seed=seed)
    W_LR = W_LR.ravel()
    ...
    for n in range(1, N):
        ...
        Uo = np.tile(u_obs[:, n][:, np.newaxis], M) - \
            np.sqrt(Gamma)@np.random.normal(size=(D, M))
