...
class TestFeatureFunctions(unittest.TestCase):
    ...
    def test_kernel_distance(self):
        mnist = load_datasets.mnist()
        dgms = mnist['zero_dim_rtl'][0:3]
        sigma=.3
        k = FF.kernel_distance(dgms, dgms, sigma)
        self.assertEqual(k.all(), np.array([4.83911125e+00, 1.12597993e-03, 2.64421959e-20, 2.15926035e+00,1.80429682e+00, 2.51962051e+01]).all())
